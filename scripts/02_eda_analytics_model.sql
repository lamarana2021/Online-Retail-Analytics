...
SELECT 
    transaction_type,
    COUNT(*) AS group_transaction
FROM analytics.vw_sales_lines
GROUP BY transaction_type
ORDER BY group_transaction DESC;

-- other tpe_transactions
SELECT *
FROM analytics.vw_sales_lines
...
SELECT *
FROM analytics.vw_sales_lines
WHERE transaction_type = 'zero priced product'
...
-- Generate all the dates between earliest and latest date using recursive cte's
DECLARE @start_date DATE;
DECLARE @end_date DATE;

SELECT 
 @start_date = MIN(CAST(invoice_date AS DATE)),
 @end_date = MAX(CAST(invoice_date AS DATE))
